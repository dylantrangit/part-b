...
# Cardinal directions as (Direction, dr, dc) tuples — lets hot paths do raw
# int arithmetic on (r, c) instead of paying for Coord/Direction attribute
# access and Coord __post_init__ bounds-check exceptions on every step.
CARDINAL_DIRS_RC = (
    (Direction.Up, -1, 0),
    (Direction.Down, 1, 0),
    (Direction.Left, 0, -1),
...
DIR_TO_DR_DC = {d: (dr, dc) for d, dr, dc in CARDINAL_DIRS_RC}
...
def generate_play_actions(state):
    ...
    for flat_idx in list(state.current_piece_list()):
        sr, sc = flat_idx >> 3, flat_idx & 7
        height = abs(int(grid[sr, sc]))
        coord = Coord(sr, sc)

        for direction, dr, dc in CARDINAL_DIRS_RC:
            tr, tc = sr + dr, sc + dc
...
# class GameState:
#     def __init__(self):
#         # Grid: 0 empty, +h red, -h blue
#         self.grid = np.zeros((BOARD_SIZE, BOARD_SIZE), dtype=np.int8)

#         # Per-colour piece lists: flat r*8+c indices
#         self.red_pieces = []
#         self.blue_pieces = []

#         # helper maps for O(1) swap-remove
#         self._red_piece_pos = {}
#         self._blue_piece_pos = {}

#         # Incrementally maintained scalars
#         self.red_tokens = 0
#         self.blue_tokens = 0
#         self.red_stacks = 0
#         self.blue_stacks = 0
...
#             # Cascade is always legal if h >= 2, regardless of what lies ahead
#             if height >= 2:
#                 yield CascadeAction(coord, direction)


# def has_any_legal_action(state):
#     for _ in generate_legal_actions(state):
#         return True
#     return False


# def is_stalemate(state):
#     return not has_any_legal_action(state)
...
#     # attacker moves to target cell, captured stack is removed,
#     # attacker height stays the same
#     state.set_cell(from_coord, 0)
#     state.set_cell(to_coord, from_value)
...
#         # token falls off board
#         if target_coord is None:
#             continue

#         # push existing stack first
#         if not state.is_empty(target_coord):
#             _push_stack(state, target_coord, direction)

#         # then place one single token
#         state.set_cell(target_coord, token_value)
